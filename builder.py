...
def create_polygon(mesh_object: Object, loop_triangle: MeshLoopTriangle) -> Polygon:
    ...
    impossible2 = math.isnan(p_base[0]) or math.isnan(p_base[1]) or math.isnan(p_base[2]) \
                  or math.isnan(p_grad_u[0]) or math.isnan(p_grad_u[1]) or math.isnan(p_grad_u[2]) \
                  or math.isnan(p_grad_v[0]) or math.isnan(p_grad_v[1]) or math.isnan(p_grad_v[2])
